...
import com.gp.updater.utils.UpdateFilter;
import com.gp.updater.utils.Utils;

public abstract class Updater extends PreferenceActivity implements
    ...
    public static final String UPDATES_CATEGORY = "updates_category";
    ...
    public ListPreference mUpdateCheck;
    public ListPreference mUpdateType;

    public PreferenceCategory mUpdatesList;
    private UpdatePreference mDownloadingPreference;

    private File mUpdateFolder;

    public boolean mStartUpdateVisible = false;
    ...
    public abstract void onStartUpdate(UpdatePreference pref);
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (DownloadReceiver.ACTION_DOWNLOAD_STARTED.equals(action)) {
                ...
                if (mProgressDialog != null) {
                    ...
                    if (count == 0) {
                        Toast.makeText(Updater.this, R.string.no_updates_found,
                                Toast.LENGTH_SHORT).show();
                    } else if (count < 0) {
                        Toast.makeText(Updater.this, R.string.update_check_failed,
	...
	public void triggerReboot(Context context) {
		// Trigger the reboot
    ...
    protected void updateUpdatesType(int type) {
        ...
        if ((Utils.getBuildType().equals("eng")) && (mUpdateType != null)) {
    ...
    private void updateLayout() {
        ...
        mUpdateFolder = Utils.getUpdateFolder();
        ...
        for (UpdateInfo update : availableUpdates) {
            // If the file exists means that is a downloaded file, set downloadURL to null to know that is a downloaded file.
            if (existingFiles.contains(update.getFileName())) {            	
            	update.setDownloadUrl(null);
            }
            
           	// add OTA updates that can be stable or nightly.
           	updates.add(update);
        }
